...
import FileDownloadIcon from "@mui/icons-material/FileDownload"; // Import icon for the button
  ...
  return (
    ...
    <Tooltip title={text} placement="bottom-start" enterDelay={500}>
      ...
      <Typography
        component="div"
        variant="body2"
        sx={{ ...truncateStyle, cursor: "default" }}
      >
        {text}
      </Typography>
    </Tooltip>
...
const SkillChipDisplay = ({
  ...
  const FullSkillsContent = (
    <Box
      sx={{ display: "flex", flexWrap: "wrap", gap: "4px", ...truncateStyle }}
    >
...
const exportCandidatesToExcel = (candidates) => {
  ...
  const excelData = candidates.map((c) => ({
    "Employee Name": c.employeeName || "N/A",
    Grade: c.grade || "N/A",
    ...
    "Skill Match %": c["skillMatch%"]
      ? Object.entries(c["skillMatch%"])
          .map(([skill, value]) => `${skill}: ${value.toFixed(2)}%`)
          .join(", ")
      : "N/A",
    "Match %": c["match%"] !== undefined ? `${c["match%"].toFixed(2)}%` : "N/A",
  }));
  ...
  XLSX.writeFile(
    workbook,
    `Shortlist_Export_${new Date().toISOString().slice(0, 10)}.xlsx`
  );
};
...
const ShortlistDisplayTable = ({
  ...
  const isAllSelected =
    ...
    selectedCandidateIds.size === candidateIds.length;
  ...
  return (
    <Box sx={{ mt: 3, maxWidth: "100%", overflowX: "auto" }}>
      {/* Title and Export Button in a Flex Container */}
      <Box
        sx={{
          display: "flex",
      ...
      >
        ...
        <Tooltip title="Export current table data as XML">
          <Button
            variant="contained"
          ...
          <TableBody>
            {isDataLoading ? (
              ...
              <TableRow>
                <TableCell
                  colSpan={headers.length}
                  sx={{ textAlign: "center", py: 8, color: "#999" }}
                >
              ...
              candidates.map((c, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell sx={{ ...cellBaseSx, fontWeight: 500 }}>
                      {c.employeeName || "N/A"}
                    </TableCell>
